...
import {addVehicle, editVehicle} from '../../utils/vehicle';
...
const EditVehicle = props => {
  ...
  const submitHandler = () => {
    ...
    editVehicle(token, body, 8)
      .then(res =>
        ToastAndroid.show('Edit vehicle success', ToastAndroid.SHORT),
      )
      .catch(err => {
        console.log(err);
        ToastAndroid.show('Edit vehicle fail', ToastAndroid.SHORT);
      });
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.addContainer}>
        ...
        <TextInput
          defaultValue={JSON.stringify(props.route.params.price)}
        ...
        <Pressable style={styles.submitButton} onPress={submitHandler}>
          <Text style={styles.textSubmitButton}>Edit Vehicle</Text>
        </Pressable>
      </ScrollView>
      <Modal
